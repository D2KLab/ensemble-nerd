...
def worker_per_file(q,objects_extractors,base_adel):
    PRECOMPUTED_SIM = {}
    flag = True
    while flag:
        ...
        if text == -1:
            ...
            for ext in extractors:
                ...
                while flag_q:
                    try:
                        if ext.name == 'adel':
                            ext.extract(text,lang=lang,setting=base_adel)
                        else:
                            ext.extract(text,lang=lang)
                        flag_q = False
                    except:
                        #if ext.name == 'dbspotlight':
                            #ext.extract(text,lang=lang)
...
for i in range(n_workers):
    p = multiprocessing.Process(target=worker_per_file, args=(queue,objects_extractors,base_adel))
    jobs.append(p)
