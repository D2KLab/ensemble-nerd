...
fasttext_path = 'training_data/'+base + '/models/fasttext/model_obj.p'
...
def built_XY_samples(features_paths,groundtruth_paths,max_fragment_len,PF,
                     features=['type', 'score', 'uris', 'fasttext'],
...
models_dict_fasttext = pickle.load(open( fasttext_path, "rb" ) )
predicted_train_fasttext = flatten_data(models_dict_fasttext['predicted_train'])
predicted_test_fasttext = flatten_data(models_dict_fasttext['predicted_test'])
extractors_disambiguation=['dandelion', 'dbspotlight', 'babelfy', 'textrazor']

X_dict_train,train_Y = built_XY_samples(features_paths_train,groundtruth_paths_train,max_fragment_len,predicted_train_fasttext,features=features,continue_flag=False,type_flag=True,extractors_types=extractors_types)

X_dict_test,test_Y = built_XY_samples(features_paths_test,groundtruth_paths_test,max_fragment_len,predicted_test_fasttext,features=features,continue_flag=False,type_flag=True,extractors_types=extractors_types)
...
def generateConcactPartSimple(X,feat,ext_name,activation_middle,reg_alpha,eg_alpha,types):
    ...
    dense_middle = Dense(dim_out, activation=activation_middle,kernel_regularizer=l2(reg_alpha), bias_regularizer=l2(reg_alpha))(input_tensor)
...
for feat in features:
    if feat in ['type','score']:
        ...
        fastext_in = Input(shape=(4,), name='fasttext')
        fastext_out = Dropout(0.3)(fastext_in)
        to_concatenate_layers.append((fastext_in,fastext_out))
