...
def getWikidataTypeRepresentation(types_ordered_list):
    ...
    resource_dict_representation = {t: [0 for i in range(n_features)] + [int(t not in WIKIDATA_CLASSES)] for t in
                                    (set(types_ordered_list) - {np.NAN})}
    ...
    for ext in extractors_names:
        if ext in type_list_dict and ext in entity_list_dict:
            type_features_obj[ext] = np.append(getTypeRepresentation(type_list_dict[ext], ontology_type_dict[ext]),
                                               getTypeRepresentation(entity_list_dict[ext], ontology_entity_dict[ext]),
                                               axis=1)
...
def getFeatures(text, lang=None, model_setting='default'):
    ...
    extractors_responses, type_list_dict, score_list_dict, entity_list_dict, ontology_type_dict, ontology_entity_dict = getAnnotations(
        text, lang=lang, model_setting=model_setting)
    ...
    if 'type' not in features_dict_all['features']:
        features_dict_all['features']['type'] = getTypeFeatures(type_list_dict, entity_list_dict, ontology_type_dict,
                                                                ontology_entity_dict)
    ...
    if 'entity' not in features_dict_all['features']:
        features_dict_all['features']['entity'], features_dict_all['features']['entity_MATRIX'] = getEntityFeatures(
            entity_list_dict, ontology_entity_dict, lang)
