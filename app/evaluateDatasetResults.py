...
features_paths = [features_folder_test + f for f in listdir(features_folder_test) if
                  isfile(join(features_folder_test, f)) and '.p' in f]
features_paths.sort()
groundtruth_paths = [path.replace(features_folder_test, ground_truth_folder).replace('.p', '.csv') for path in
                     features_paths]
...
extractors_type_lists = {key: reduce(lambda x, y: x + y, extractors_type_lists_per_file[key]) for key in
                         extractors_type_lists_per_file}
...
generateHistogram(for_hist_micro, evaluation_folder_NER_bin + 'micro_hist.png',
                  extractors_type + ['ensemble', 'esemble_raw'])
generateHistogram(for_hist_macro, evaluation_folder_NER_bin + 'macro_hist.png',
                  extractors_type + ['ensemble', 'esemble_raw'])


def generateBRATFiles(output_folder, segments_names, texts_paths, offsets_per_file, types_per_file,
                      texts_tokens_per_file):
...
def formExtractorsBrat(features_paths, extractors_type_lists_per_file, types, brat_extractors_folder):
    ...
    for z, ext in enumerate(extractors_type_lists_per_file):
        for i, f in enumerate(extractors_type_lists_per_file[ext]):
            ...
            out_path = brat_extractors_folder + '/' + ext + '/' + features_paths[i].split('/')[-1].split('.')[
                0] + '.ann'
...
standard_gold_paths = [brat_truth_folder + f for f in listdir(brat_truth_folder) if
                       isfile(join(brat_truth_folder, f)) and '.ann' in f]
...
extractors_entity_lists = {key: reduce(lambda x, y: x + y, extractors_entity_lists_per_file[key]) for key in
                           extractors_entity_lists_per_file}
