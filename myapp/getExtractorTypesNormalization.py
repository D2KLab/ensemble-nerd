...
def built_XY_samples(features_paths,groundtruth_paths,max_fragment_len,
    ...
    for i,f_p in enumerate(features_paths):
        ...
        Y_file = built_Y_sample(ground_truth_pd,extractors_disambiguation,continue_flag=continue_flag,type_flag = type_flag)
        #print(Y_file)
        #print(list(X_file[0]))
        if i!=0:
...
for ext in extractors_types:
    train_X,train_Y = built_XY_samples(features_paths_train,groundtruth_paths_train,max_fragment_len,features=['type'],continue_flag=continue_flag,type_flag=type_flag,extractors_types=[ext])
    test_X,test_Y = built_XY_samples(features_paths_test,groundtruth_paths_test,max_fragment_len,features=['type'],continue_flag=continue_flag,type_flag=type_flag,extractors_types=[ext])
    ...
    model.fit(model_obj['train_X'], model_obj['train_Y'], epochs=epochs, batch_size=batch,callbacks=[TestCallback(model_obj, gt_test,max_fragment_len,inv_types_map,types,saving_folder),early_stop],validation_data = (model_obj['test_X'], model_obj['test_Y']))
